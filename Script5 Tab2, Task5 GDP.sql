...
WHERE c.region_in_world IN ('British Isles', 'Western Europe', 'Southern Europe', 'Central and Southeast Europe', 'Baltic Countries', 'Nordic Countries', 'Eastern Europe') AND e.year BETWEEN 2000 AND 2021
ORDER BY year_E; 

-- SLOUČENÍ S PRVNÍ TABULKOU:

CREATE TABLE t_Lenka_Doova_project_SQL_secondary_final AS
SELECT * FROM t_Lenka_Doova_project_SQL_primary_final t1f
LEFT JOIN t_economies_countries tec ON tec.year_E = t1f.year_W

SELECT * FROM t_Lenka_Doova_project_SQL_secondary_final;
